...
if ($Task -eq 'Publish') {
    $null = PowerShell -Command {Get-PackageProvider -Name NuGet -ForceBootstrap; Install-Module -Name PowerShellGet -Force;}
}
...
'BuildHelpers', 'psake' | Resolve-Module -UpdateModules:($PSBoundParameters.ContainsKey('UpdateModules')) -Verbose
